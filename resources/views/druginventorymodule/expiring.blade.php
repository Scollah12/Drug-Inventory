...
<div class="text-center">
  ...
  <form action="{{ route('medicines.expiry.filter') }}" method="POST" class="mb-4 d-inline-block">
    @csrf
  ...
  @if(isset($expiringMeds) && $expiringMeds->count() > 0)
        ...
        <tbody>
          @foreach($expiringMeds as $med)
            ...
            <tr class="{{ $rowClass }}">
              ...
              <td>{{ $med->quantity }}</td>
              <td>{{ $med->category }}</td>
              <td>{{ $med->expiry_date->format('Y-m-d') }}</td>
              <td>
